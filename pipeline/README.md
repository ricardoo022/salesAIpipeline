...
The `qualification/` package (see `pipeline/qualification/README.md`) consumes
the existing JSON outputs without rerunning transcription or signal
extraction. Its hierarchical chunking layer is implemented through US-1.3:

- `TranscriptSegment` preserves one original speaker utterance and its word
  timestamps (US-1.1, `normalize_transcript()`).
- `ConversationSection` groups chronological segments, usually from both
  speakers (US-1.2, `create_sections()`).
- `ExtractionChunk` renders speaker-labeled text for the LLM and retains
  source IDs (US-1.3, `create_chunks()`).
- `ChunkingConfiguration` records time windows, token limits, turn overlap,
  and tokenizer identity — implemented alongside the schemas above.
- `CoverageRecord` (proving that no transcript segment was omitted) remains
  planned work (US-1.5).
